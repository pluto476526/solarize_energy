/* controller.c — improved, hardened version (function signatures preserved) */
...
void controller_update_measurements(system_controller_t* ctrl) {
    ...
    /* Grid handling: compute grid_power = consumption - generation - battery */
    ...
    /* If battery voltage is missing, leave it to battery_update_measurements.
       Do not overwrite valid measurements. This avoids hiding sensor errors. */
    if (ctrl->measurements.battery_voltage <= 0.0) {
        /* no-op: keep whatever battery_update_measurements set (or zero if missing) */
    }
...
void controller_handle_faults(system_controller_t* ctrl) {
    ...
    if (new_faults != 0) {
        ...
        /* Use snprintf to build description and avoid trailing newline from ctime */
        char tsbuf[32];
        struct tm tm_info;
...
/* Nicely print a summary of system status */
...
/* Verify safety limits - return false if system must stop */
